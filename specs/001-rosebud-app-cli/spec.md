...
- Q: Python项目应该使用什么工具进行依赖管理和项目初始化？ → A: 使用 uv init 初始化项目，uv 管理依赖和打包发布
- Q: 默认的AI大模型应该选择什么？ → A: gemma3:270m（体积最小，运行效率高，适合CLI场景）
- Q: AI Agent的核心功能是什么？ → A: 只问出高质量问题，不提供回答（专注问题生成）
- Q: AI生成问题时应该重点参考什么？ → A: 昨日用户对问题的回答（重要的上下文输入）
...
3. **Given** 用户选择时区和AI模型（默认gemma3:270m），**When** 配置保存，**Then** 系统应该验证设置的有效性并提供确认反馈
...
3. **Given** 用户回答问题，**When** 对话结束，**Then** 系统应该将用户的回答保存到当天的日记文件中（用于后续问题生成参考）
...
- **FR-001**: 系统必须允许用户通过初始化命令设置个人配置，包括存储路径、时区、AI模型（默认gemma3:270m）和提醒时间
- **FR-002**: 系统必须支持通过日志记录命令将内容以 `HH:MM:SS 用户内容` 格式实时写入当天的日记文件（格式：YYYY-MM-DD.md）
- **FR-003**: 系统必须能够存储和管理用户的长期记忆信息，通过记忆管理命令添加
- **FR-004**: 系统必须能够基于今日记录、长期记忆和昨日回答生成1个个性化问题，通过智能问答命令触发（AI只问问题，不提供回答）
...
- **用户配置**: 包含 daybook 文件夹路径、时区、默认AI模型（gemma3:270m）、每日提醒时间
- **日记条目**: 按日期组织的文本文件，包含带时间戳的用户记录（HH:MM:SS 格式）、AI问题和回答
- **长期记忆**: 用户提供的个人信息、目标和偏好，用于AI个性化
- **AI问题**: 基于当日内容、长期记忆和昨日回答生成的个性化反思问题（用户主动交互时1个，定时任务时3个；AI只问问题，不提供回答）
- **用户回答**: 用户对AI问题的回答，作为重要上下文保存用于后续问题生成
